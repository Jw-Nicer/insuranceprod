...
import {
  ArrowUpRight, Search, RefreshCw, LayoutGrid, List as ListIcon,
  Bookmark, BookmarkCheck, Copy as CopyIcon, Filter, CalendarDays,
} from "lucide-react";
 ...
 ************************************/
interface NewsItem {
  title: string; pubDate: string; link: string; guid: string; author: string;
  thumbnail: string; description: string; content: string;
  enclosure: Record<string, unknown>; categories: string[];
}
type EnrichedNews = NewsItem & { _sourceKey: string; _sourceName: string; };
interface SourceDef { key: string; name: string; url: string; region?: string; notes?: string; }
...
const L = {
  ...
  search: "Search news…",
  sort: "Sort", sortBy: "Sort by", newest: "Newest first", oldest: "Oldest first", az: "A → Z", za: "Z → A",
  all: "All", sources: "Sources", topics: "Topics", read: "Read Full Article", copy: "Copy link", refresh: "Refresh",
  emptyTitle: "No articles match", emptyDesc: "Try adjusting your search, filters, sources, or topics.",
  errorTitle: "Error", errorDesc: "Could not load the news feeds.",
  loading: "Loading news…", saved: "Saved", unsaved: "Removed bookmark",
...
const LOB_KEYWORDS: Record<string, string[]> = {
  ...
  Flood: ["flood", "nfip"], Parametric: ["parametric"],
};
...
const REGION_KEYWORDS: Record<string, string[]> = {
  "US - California": ["california"], "US - Florida": ["florida"], "US - Texas": ["texas"], "US - New York": ["new york"],
  "US - National": ["united states","u.s."," us "], Canada: ["canada","canadian"],
...
function anyIncludes(text: string, keywords: string[]) { const t = lower(text); return keywords.some(k => t.includes(lower(k))); }
function matchFromKeywords(map: Record<string,string[]>, text: string) {
  const hits: string[] = []; for (const [label, words] of Object.entries(map)) if (anyIncludes(text, words)) hits.push(label); return hits;
}
...
function buildTopicIndex(items: EnrichedNews[]) {
  ...
  for (const it of items) { const uniq = new Set(deriveMeta(it).topics); for (const t of uniq) counts.set(t, (counts.get(t) || 0) + 1); }
...
export default function InsuranceNewsClient() {
  ...
  const idFor = (item: NewsItem) => (item.guid || item.link || item.title).replace(/https?:\/\//, "").toLowerCase();
      ...
      try {
        const results = await Promise.allSettled(
          active.map(async (src) => {
            ...
            const items: EnrichedNews[] = (data.items || []).map((it: NewsItem) => ({
              ...it, _sourceKey: src.key, _sourceName: src.name,
            }));
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if (r.status === "fulfilled") {
            ...
            const src = active[i];
            if (src) status.push({ key: src.key, ok: false, count: 0, error: (r as any).reason?.message || "Fetch failed" });
          }
        }
  ...
  const filtered = React.useMemo(() => {
    ...
    let list = news.filter((n) => {
      ...
      const textMatch = !q || n.title.toLowerCase().includes(q) || stripHtml(n.description).toLowerCase().includes(q);
    ...
    switch (sort) {
      case "newest": list = list.sort((a, b) => +new Date(b.pubDate) - +new Date(a.pubDate)); break;
      case "oldest": list = list.sort((a, b) => +new Date(a.pubDate) - +new Date(b.pubDate)); break;
  ...
  return (
    <>
      ...
      <div className="mb-6 sm:mb-8">
          ...
          <div className="flex gap-2">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="whitespace-nowrap">{L.sources}</Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-64">
                <DropdownMenuLabel>Active feeds</DropdownMenuLabel>
                <DropdownMenuSeparator />
                {RSS_SOURCES.map((s) => (
                  <DropdownMenuCheckboxItem key={s.key} checked={activeSourceKeys.includes(s.key)} onCheckedChange={() => toggleActiveSource(s.key)}>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" onClick={() => setReload((r) => r + 1)} aria-label={L.refresh}>
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            {mounted && lastUpdated && <span className="hidden sm:inline">Updated {formatRelative(lastUpdated)}</span>}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild><Button variant="outline" className="whitespace-nowrap">{L.sort}</Button></DropdownMenuTrigger>
          ...
          <ScrollArea className="w-full whitespace-nowrap">
            <div className="flex items-center gap-2 pb-2">
              {sourceChips.map(({ source, count }) => (
                <Button key={source} size="sm" variant={selectedSource === source ? "secondary" : "outline"} className="rounded-full" onClick={() => setSelectedSource(source)}>
                  {source}
          ...
          <ScrollArea className="w-full whitespace-nowrap">
            <div className="flex items-center gap-2 pb-2">
              {topicChips.map(({ topic, count }) => (
                <Button key={topic} size="sm" variant={selectedTopic === topic ? "secondary" : "outline"} className="rounded-full" onClick={() => setSelectedTopic(topic)}>
                  {topic}
                  <Badge variant="secondary" className="ml-2 rounded-full px-1.5 text-[10px]">{count}</Badge>
                </Button>
              ))}
      ...
      {loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 sm:gap-6 mt-6" aria-busy>
          {Array.from({ length: 9 }).map((_, i) => (
            <Card key={i}>
              <CardHeader><Skeleton className="h-5 w-3/4" /><Skeleton className="mt-2 h-4 w-1/3" /></CardHeader>
              <CardContent><Skeleton className="h-36 w-full rounded-lg" /><Skeleton className="mt-3 h-4 w-full" /><Skeleton className="mt-2 h-4 w-5/6" /></CardContent>
      ...
      {!loading && error && (
        <Card className="mt-6">
          <CardHeader><CardTitle className="text-destructive">{L.errorTitle}</CardTitle><CardDescription>{L.errorDesc}</CardDescription></CardHeader>
          <CardContent><p className="text-sm text-muted-foreground">{error}</p></CardContent>
          <CardFooter><Button onClick={() => setReload((r) => r + 1)}>{L.refresh}</Button></CardFooter>
        </Card>
      )}
...
function TopicBadges({ meta }: { meta: ReturnType<typeof deriveMeta> }) {
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-1.5" aria-label="Tags">
      {chips.map((t) => (
        <Badge key={t} variant="outline" className="rounded-full text-[10px] px-2 py-0.5">{t}</Badge>
      ))}
    </div>
  );
}
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1 inline-flex items-center gap-1">
              ...
              <span>{formatAbsolute(item.pubDate)} {mounted && `• ${formatRelative(item.pubDate)}`}</span>
            </CardDescription>
      ...
      <CardFooter className="gap-2">
        <Button asChild variant="outline" className="w-full">
          <a href={item.link} target="_blank" rel="noopener noreferrer" aria-label={`${L.read}: ${item.title}`}>
            {L.read}<ArrowUpRight className="ml-2 h-4 w-4" />
          </a>
        </Button>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild><Button variant="ghost" size="icon" aria-label={L.copy} onClick={onCopy}><CopyIcon className="h-4 w-4" /></Button></TooltipTrigger>
  ...
  return (
    <Card className="border-dashed">
      <CardContent className="py-16 text-center">
        <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-2xl bg-primary/10"><Filter className="h-6 w-6" /></div>
